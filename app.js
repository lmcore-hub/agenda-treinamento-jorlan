(function () {
  ...
  async function setupBookingPage() {
    ...
    const storeSelect = form ? form.querySelector('select[name="store"]') : null;
    const citySelect = form ? form.querySelector('select[name="city"]') : null;
    if (storeSelect && citySelect) {
      const syncCityFromStore = () => {
        const selected = storeSelect.options[storeSelect.selectedIndex];
        const city = selected ? selected.dataset.city : "";
        if (city) citySelect.value = city;
      };
      storeSelect.addEventListener("change", syncCityFromStore);
      syncCityFromStore();
    }
